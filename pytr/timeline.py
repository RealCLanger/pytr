...
class Timeline:
    def __init__(self, tr, output_path, not_before, not_after, dump_raw_data_events=True, dump_raw_data_all=False):
        self.tr = tr
        self.output_path = output_path
        self.not_before = not_before
        self.not_after = not_after
        self.dump_raw_data_events = dump_raw_data_events
        self.dump_raw_data_all = dump_raw_data_all
    ...
    def finish_timeline_details(self):
        self.log.info("Received all event details, updating event database...")
        if self.skipped_detail > 0:
            self.log.warning(f"Skipped {self.skipped_detail} unsupported events")

        if self.dump_raw_data_all:
            with (self.output_path / "timeline_details.json").open("w") as f:
        ...
        if old_events:
            ...
            for event in self.events:
                ...
                self.events = sorted(cur_events.values(), key=lambda value: value["timestamp"])

        if self.dump_raw_data_events:
