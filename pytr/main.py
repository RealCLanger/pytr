...
def get_main_parser():
    ...
    info = (
        ...
        + " Optionally, more raw data can be exported."
    )
    ...
    parser_export_transactions.add_argument("--dump-raw-data-events", default=True, help="Dump raw data of parseable events in json format (all_events.json)", action=argparse.BooleanOptionalAction)
    parser_export_transactions.add_argument("--dump-raw-data-all", default=False, help="Dump all raw data from timeline in json format", action=argparse.BooleanOptionalAction)
    parser_export_transactions.add_argument(
        "--format",
        choices=("json", "csv"),
        default="csv",
...
def main():
    ...
    if args.command == "login":
        ...
        tl = Timeline(
            ...
            not_before,
            not_after,
            args.dump_raw_data_events,
            args.dump_raw_data_all,
        )
        asyncio.get_event_loop().run_until_complete(tl.tl_loop())
        events = tl.events

        with (
            (args.outputdir / ("account_transactions." + args.format)).open("w", encoding="utf-8")
